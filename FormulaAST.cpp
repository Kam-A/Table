...
public:
    ...
    double Evaluate(std::function<std::variant<double, FormulaError>(Position)> func) const override {
        ...
        switch (type_) {
            case Add:
                result = SafeAdd(lhs_->Evaluate(func), rhs_->Evaluate(func));
                break;
            case Subtract:
                result = SafeAdd(lhs_->Evaluate(func), - rhs_->Evaluate(func));
                break;
            case Multiply:
                result = SafeMultyplicate(lhs_->Evaluate(func), rhs_->Evaluate(func));
                break;
            case Divide:
                result = SafeDivide(lhs_->Evaluate(func), rhs_->Evaluate(func));
                break;
            default:
                // have to do this because VC++ has a buggy warning
                assert(false);
